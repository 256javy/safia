  ...
  if (action === "disable") {
    return {
      ...
      onComplete: async (_data, ctx) => {
        ...
        const store = useAccountsStore.getState();
        const r = store.disableTotp(ctx.account.id);
        if (!r.ok) throw new Error("totp.forbidden");
      },
    };
  }
    ...
    screens: [
      {
        ...
        next: () => "verify",
        render: ({ ctx, advance }: ScreenRenderArgs) => <TotpScanScreen ctx={ctx} onContinue={() => advance()} />,
...
function TotpScanScreen({ ctx, onContinue }: { ctx: ScreenRenderArgs["ctx"]; onContinue: () => void }) {
  const secret = ctx.data[SECRET_KEY] ?? "";
  const label = ctx.account?.identity.email ?? ctx.account?.identity.username ?? "user";
  const uri = secret ? otpauthUri({ secret, label, issuer: "Facebook (Safia)" }) : "";
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="w-full rounded-md border border-current/15 bg-current/5 p-3 text-center">
        <p className="text-[11px] uppercase tracking-wider text-current/60">{ctx.tp("totp.enable.scan.secretLabel")}</p>
        <p className="mt-1 break-all font-mono text-xs text-current">{secret.match(/.{1,4}/g)?.join(" ")}</p>
      </div>
      <button
        type="button"
        onClick={onContinue}
        style={{ background: "var(--flow-primary-bg, #1877f2)", color: "var(--flow-primary-text, #fff)" }}
