  ...
  if (action === "disable") {
    return {
      ...
      onComplete: async (_data, ctx) => {
        ...
        const r = useAccountsStore.getState().disableTotp(ctx.account.id);
...
function TotpScanScreen({
  ctx,
  onContinue,
}: {
  ctx: ScreenRenderArgs["ctx"];
  onContinue: () => void;
}) {
  const secret = ctx.data[SECRET_KEY] ?? "";
  const label = ctx.account?.identity.username ?? ctx.account?.identity.email ?? "user";
  const uri = secret ? otpauthUri({ secret, label, issuer: "Instagram (Safia)" }) : "";
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <button
        ...
        onClick={onContinue}
        style={{
          background: "var(--flow-primary-bg, #0095F6)",
